...
class FORCEReadout(Linear):
    def __init__(self, *args, lmbd=1.0, initialize_with_zero=True, **kwargs):
        ...
        self.P = np.eye(self.input_dim, dtype=self.dtype) * (1 / lmbd)  # create initial value of P
...
def emulate_online(time_steps, x0, net, w_feed, w_out, ds=None, train_range=None, force_every=1, display=True):
...
def dataset():
    random.seed(738463892674817263)
    ...
    for _idx in range(20):
        ...
        yield to_args(time_steps, x0, net, w_feed, dim, dim_in, ds=ds, train_range=[25, 75], display=False)
